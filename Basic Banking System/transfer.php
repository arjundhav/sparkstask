...
$username="root";
$password="";
$con=mysqli_connect($server,$username,$password,"arjun");
if(!$con){
...
$sql = "SELECT balance FROM arjun WHERE name='$sender'";
...
if ($result->num_rows > 0) {
 ...
 if($amount>$row["balance"] or $row["balance"]-$amount<100){
  ...
  header("Location: $location&message=transactionDenied");
    ...
    $sql = "UPDATE `arjun` SET balance=(balance-$amount) WHERE name='$sender'";
...
if($flag==true){
$sql = "UPDATE `arjun` SET balance=(balance+$amount) WHERE name='$receiver'";
...
if($flag==true){
    $sql = "SELECT * from arjun where name='$sender'";
    $result = $con-> query($sql);
    while($row = $result->fetch_assoc())
     {
         $s_acc=$row['ac'];
...
//  Transcation DEatiled Stored in the DB
 ...
 $sql = "SELECT * from arjun where name='$receiver'";
 $result = $con-> query($sql);
 while($row = $result->fetch_assoc())
  {
      $r_acc=$row['ac'];
}        
$sql = "INSERT INTO `transfer`.`transfer` (`s_name`, `s_acc_no`, `r_name`, `r_acc_no`, `amount`) VALUES ('$sender', '$s_acc', '$receiver', '$r_acc', '$amount');";
if ($con->query($sql) === TRUE) {
} else 
{
