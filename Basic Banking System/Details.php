...
<body>
  ...
  <table>
    ...
    $username = "root";
    $password = "";
    $conn = mysqli_connect($server, $username, $password, "arjun");
    if (!$conn) {
    ...
    if (isset($_SESSION['user'])) {
      ...
      $sql = "SELECT * FROM arjun WHERE name='$user'";
      $result = mysqli_query($conn, $sql);

      while ($row = mysqli_fetch_array($result)) {
        echo "<tr>";

        echo "<td>" .$row["ac"] . "</td><td>" . $row["name"] . "</td>
              <td>" . $row["email"] . "</td><td>" . $row["account"] . "</td><td>" . $row["balance"] . "</td>";
    ...
    <div class="card container">
      ...
      <form action="transfer.php" method="POST">
        ...
        <select name="reciever" id="dropdown" class="textbox" required>
          ...
          <?php
          $db = mysqli_connect("localhost", "root", "", "arjun");
          $res = mysqli_query($db, "SELECT * FROM arjun WHERE name!='$user'");
